...
extension CGAffineTransform {
    ...
    /// - Returns: The transform that will make the `fromSize` parameter fit (aspect fill) inside the `toSize` parameter.
    ...
    ///
    /// - Parameters:
    ///   - fromSize: The size that needs to be transformed to fit (aspect) in the other given size.
    ///   - toSize: The size that should be matched by the `fromSize` parameter.
    /// - Returns: The transform that will make the `fromSize` parameter fit (aspect) inside the `toSize` parameter.
    static func scaleTransform(forSize fromSize: CGSize, aspectInSize toSize: CGSize) -> CGAffineTransform {
        let scale: CGFloat = (fromSize.width >= fromSize.height)
